...
class WechatReceive extends Common {
    ...
    public function sendCustomMessage($data) {
        ...
        $result = $this->httpRequest(self::API_URL_PREFIX . self::CUSTOM_SEND_URL . "access_token={$this->access_token}", 'POST', json_encode($data));
        if ($result) {
    ...
    public function addCunstom($data)
    {
        if (!$this->access_token && !$this->get_access_token()) {
            return false;
        }
        $result = $this->httpRequest('https://api.weixin.qq.com/customservice/kfaccount/add?'. "access_token={$this->access_token}", json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function setTMIndustry($id1, $id2 = '') {
        ...
        $result = $this->httpRequest(self::API_URL_PREFIX . self::TEMPLATE_SET_INDUSTRY_URL . "access_token={$this->access_token}", 'POST', json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                return $json;
    ...
    public function getTemplateList()
    {
        if (!$this->access_token && !$this->get_access_token())
            return false;
        $result = $this->httpRequest('https://api.weixin.qq.com/cgi-bin/template/get_all_private_template?' . "access_token={$this->access_token}", 'GET');
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function addTemplateMessage($tpl_id) {
        ...
        if (!$this->access_token && !$this->get_access_token())
            return false;
        $result = $this->httpRequest(self::API_URL_PREFIX . self::TEMPLATE_ADD_TPL_URL . "access_token={$this->access_token}", 'POST', json_encode($data));
        if ($result) {
//            file_put_contents('glog.txt', $result);
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function sendTemplateMessage($data) {
        ...
        $result = $this->httpRequest(self::API_URL_PREFIX . self::TEMPLATE_SEND_URL . "access_token={$this->access_token}", 'POST', json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                return $json;
    ...
    public function sendMassMessage($data) {
        ...
        $result = $this->httpRequest(self::API_URL_PREFIX . self::MASS_SEND_URL . "access_token={$this->access_token}", json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                return $json;
    ...
    public function sendGroupMassMessage($data) {
        ...
        $result = $this->httpRequest(self::API_URL_PREFIX . self::MASS_SEND_GROUP_URL . "access_token={$this->access_token}", json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                return $json;
    ...
    public function deleteMassMessage($msg_id) {
        ...
        $result = $this->httpRequest(self::API_URL_PREFIX . self::MASS_DELETE_URL . "access_token={$this->access_token}", json_encode(array('msg_id' => $msg_id)));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                return $json;
    ...
    public function previewMassMessage($data) {
        ...
        $result = $this->httpRequest(self::API_URL_PREFIX . self::MASS_PREVIEW_URL . "access_token={$this->access_token}", json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                return $json;
    ...
    public function queryMassMessage($msg_id) {
        ...
        $result = $this->httpRequest(self::API_URL_PREFIX . self::MASS_QUERY_URL . "access_token={$this->access_token}", json_encode(array('msg_id' => $msg_id)));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                return $json;
     ...
     * @param $contentStr 内容
     */
    public function sendText($contentStr='', $ToUserName='', $FromUserName='')
    {
        $FromUserName = $FromUserName ? $FromUserName : $this->getRevTo();
        $textTpl = "<xml>
     ...
     * @param $Url
     */
    public function sendImg($ToUserName='', $FromUserName='', $Title='', $Description='', $PicUr='', $Url='')
    {
        $FromUserName = $FromUserName ? $FromUserName : $this->getRevTo();
        $textTpl = "<xml>
                                <ToUserName><![CDATA[%s]]></ToUserName>
        ...
        $resultStr = sprintf($textTpl,$ToUserName, $FromUserName, time(), 'news', '1',$Title, $Description,  $PicUr,  $Url);
        exit($resultStr);
    }
     ...
     * @param $FromUserName
     * @param $url  跳转地址
     * @param array $first  描述
     * @param $orderMoneySum 支付金额
     * @param $orderProductName 商品信息
     * @param array $remark 备注
     * @return array|bool
     */
    public function senTempOrderPay($FromUserName, $url, $first = array('value'=>'','color'=>''), $orderMoneySum, $orderProductName, $remark = array('value'=>'','color'=>''))
    {
        $FromUserName = $FromUserName ? $FromUserName : $this->getRevFrom();
        $data = array(
            'touser' => $FromUserName,
            'template_id' => '2HD9Lh0Oll1nc4ef_U8Nnhzf3xPFahnjAMEEXhbBWsU',
            'url' => $url,
            'topcolor' => '#FF0000',
            'data' => array(
                'first' => array(
        ...
        return $this->sendTemplateMessage($data);
    }
     ...
     * @return array|bool
     */
    public function senTempOrderStatus($FromUserName, $url, $first = array('value'=>'','color'=>''), $OrderSn, $OrderStatus, $remark = array('value'=>'','color'=>''))
    {
        $FromUserName = $FromUserName ? $FromUserName : $this->getRevFrom();
        $data = array(
            'touser' => $FromUserName,
            'template_id' => 'WBTw6jeUFs_EW8pq1zkmBZ4R3QSe9_nt-AZOgzkKwM0',
            'url' => $url,
            'topcolor' => '#FF0000',
            'data' => array(
                'first' => array(
     ...
     * @return array|bool
     */
    public function senTempRefund($FromUserName, $url, $first = array('value'=>'','color'=>''), $reason, $refund, $remark = array('value'=>'','color'=>''))
    {
        $FromUserName = $FromUserName ? $FromUserName : $this->getRevFrom();
        $data = array(
            'touser' => $FromUserName,
            'template_id' => 'SITzwC_ioGXLSmnqc5glZy8W9REfbwsGKj9oFI10Cic',
            'url' => $url,
            'topcolor' => '#FF0000',
            'data' => array(
                'first' => array(
     ...
     * @param $FromUserName
     * @param $url
     * @param array $result 描述
     * @param $withdrawMoney 金额
     * @param $withdrawTime 领奖时间
     * @param $cardInfo 银行信息
     * @param $arrivedTime 到账时间
     * @param array $remark
     * @return array|bool
     */
    public function senTempBonus($FromUserName, $url, $result = array('value'=>'','color'=>''), $withdrawMoney, $withdrawTime, $cardInfo, $arrivedTime, $remark = array('value'=>'','color'=>''))
    {
        $FromUserName = $FromUserName ? $FromUserName : $this->getRevFrom();
        $data = array(
            'touser' => $FromUserName,
            'template_id' => 'zyMH6beOXX_3belw55XqFo5LuBoLFPKSGDfvHaCuW_k',
            'url' => $url,
            'topcolor' => '#FF0000',
            'data' => array(
                'result' => array(
